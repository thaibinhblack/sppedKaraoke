            ...
            <v-col cols="12" sm="6" md="3">
                ...
                <ul class="list list-province">
                    <li v-for="(province,index) in karaoke_province" :key="index">
                        <a v-if="index <= 5" :href="'/karaoke?ID_PROVINCE='+province.ID_PROVINCE">
...
export default {
    ...
    methods: {
        api_get_province()
        {
            this.$http.get(this.$store.state.API_URL + 'karaoke?sort=groupby').then((response) => {
                this.karaoke_province = response.data
            })
        }
    },
    created() {
        this.api_get_province()
    },
}
</script>
...
.list-province li a:hover {color: #f65e39 !important;text-decoration: none ;padding: 50ppx 10px 10px 10px;}
</style>