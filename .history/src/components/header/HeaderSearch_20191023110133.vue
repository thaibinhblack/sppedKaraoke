        ...
        .then(coordinates => {
            ...
            .then((response) => {
                ...
                const test = this.provinces.filter((value,index,array) => {
                   console.log(correctRegex.test(array[index].NAME_PROVINCE))
                   return  array[index].NAME_PROVINCE.search(response.data.results[0].components.city)
                })
                console.log('test',response.data.results[0].components.city,test,correctRegex,this.provinces);
            })
