<template>
     <v-toolbar>
                <v-row  no-gutters dense style="padding: 15px;width:100%">
                    <v-col cols="12" md="3">
                        <h3><span><v-icon class="btn-menu" v-if="$session.has('user_social')" :class="{active: menuManager}" @click="closeOpenSidebar()" >mdi-menu</v-icon></span> SPEED KARAOKE</h3>
                    </v-col>
                
                    <v-col cols="12" md="9">
                        <ul v-if="!$session.has('user_social')">
