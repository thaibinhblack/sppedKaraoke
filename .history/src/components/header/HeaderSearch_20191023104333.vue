...
export default {
    ...
    created()
    {
        ...
        .then(coordinates => {
            ...
            .then((response) => {
                 const correctRegex = new RegExp(response.data.results[0].components.city+'\\b');
                const test = this.provinces.filter((value,index,array) => {
                    return array[index].NAME_PROVINCE.includes(response.data.results[0].components.city)
                })
                console.log('test',response.data.results[0].components.city,test,correctRegex);
            })
