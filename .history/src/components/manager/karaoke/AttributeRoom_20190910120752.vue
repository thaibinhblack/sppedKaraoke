                ...
                <v-card>
                    <v-card-title primary-title>
                        <v-row>
                            ...
                            <v-col cols="12" sm="12">
                                <button class="btn-add title-label" @click="show = !show" ><v-icon>mdi-plus</v-icon> Thêm mới</button>
                    ...
                    <v-card-text>
                        ...
                        <template v-slot:item.ACTION="{item}">
                            ...
                            <button class="btn-action"><v-icon>mdi-delete</v-icon></button>
                        </template>
                    </v-data-table>
                    </v-card-text>  
                </v-card>
...
export default {
    ...
    methods: {
        ApiGetAttributeRoom(){
            this.$http.get(this.$store.state.API_URL + 'attribute-room?UUID_ROOM_BAR_KARAOKE='+this.room.UUID_ROOM_BAR_KARAOKE)
            .then((response) => {
        ...
        createAttribute()
        {
            ...
            else
            {
                ...
                {
                    data.append("USER_CREATE",this.$session.get("user_social").email)
                }
                this.$http.put(this.$store.state.API_URL + 'attribute-room/'+this.attribute.UUID_ATTRIBUTE_ROOM,this.attribute)
                .then(() => {
                    this.message.type = 'success'
