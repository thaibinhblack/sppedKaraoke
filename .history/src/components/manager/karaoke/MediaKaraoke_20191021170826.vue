<template>
    <v-col cols="12" sm="12" v-if="images.length > 0">
                <v-card>
                    ...
                    <v-card-text>
                        <v-row v-if="room == true">
                            <v-col cols="12" sm="3" md="2" style="position:relative">
                                ...
                                <v-img v-if="IMAGE_ROOM != null" width="100%" ref="imageRoom" height="150px" :src="$store.state.PUBLIC_URL + IMAGE_ROOM"></v-img>
                                <v-img v-else width="100%" ref="imageRoom" height="150px" style="background:#e2e2e2;"></v-img>
                        ...
                        <v-row>
                            <v-col cols="12" sm="3" md="2" v-for="(image,i) in images" :key="i">
                                <div class="item-image" @click="selected = image.URL_IMAGE">
                ...
                <media-full :dialog="dialog" :selected="selected" />
            </v-col>
</template>
...
export default {
    ...
    methods: {
        ...
        uploadImageRoom()
        {
            ...
            this.$http.post(this.$store.state.API_URL + 'room/'+this.$route.params.uuid_room+'?api_token='+this.$session.get('token'),data)
            .then((response) => {
