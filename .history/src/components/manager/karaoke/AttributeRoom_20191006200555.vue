    ...
    methods: {
        ...
        createAttribute()
        {
            if(this.update == false)
            {
                ...
                data.append("UUID_ROOM_BAR_KARAOKE",this.$route.params.uuid)
                this.$http.post(this.$store.state.API_URL + 'attribute-room?api_token='+this.$session.get('token'),data).then(() => {
        ...
        {
            alert('123')
            this.$http.delete(this.$store.state.API_URL + 'attribute-room/'+uuid_attribute).then(() => {
                this.ApiGetAttributeRoom()
            })
        },
        btnUpdate(item)
