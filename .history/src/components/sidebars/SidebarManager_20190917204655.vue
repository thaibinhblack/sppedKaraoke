<template>
    <v-card class="siebar-manager-karaoke" :class="{active: activeSidebar}">
            <v-navigation-drawer permanent width="100%" > 
                <v-list-item two-line="" v-if="$session.has('user_social')">
                    <v-list-item-avatar>
                        <v-img :src="user.AVATAR != null ?  $store.state.PUBLIC_URL + user.AVATAR: ''"></v-img>
                    </v-list-item-avatar>
                    <v-list-item-content>
                 ...
                 <v-list-item two-line="" v-else>
                    <v-list-item-avatar>
                        <v-img :src="$session.get('user').AVATAR != null ?  $session.get('user').AVATAR: ''"></v-img>
                    </v-list-item-avatar>
                    <v-list-item-content>
                        <v-list-item-title class="title">
                            <span style="font-size:14px;">
                                {{$session.get('user').DISPLAY_NAME != null ? $session.get('user').DISPLAY_NAME : ''}}
                            </span>
                            
                        </v-list-item-title>
                        <v-list-item-subtitle>
                            <small>{{$session.get('user').EMAIL != null ? $session.get('user').EMAIL : ''}}</small>
                        </v-list-item-subtitle>
                    </v-list-item-content>
                </v-list-item>
                <v-divider></v-divider>
...
export default {
    ...
    methods: {
        ApiCheckToken()
        {
            this.$http.get(this.$store.state.API_URL + 'user/check?api_token='+this.$session.get('token')).then((response) => {
