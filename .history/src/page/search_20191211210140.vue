                       ...
                       <li>
                        ...
                        >
                            <template v-slot:prepend>
                                <span>{{range[0].toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')}} vnđ/h</span>
                            </template>
                            <template v-slot:append>
                                 <span>{{range[1].toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')}} vnđ/h</span>
                            </template>
                        </v-range-slider>
                        </li>
                        <li style="margin-top:10px;"><v-btn outline color="primary" dark style="width:100%" @click="search()">TÌM KIẾM</v-btn></li>
                   </ul>
               </v-col>
...
export default {
    ...
    {
        '$route': {
            handler: 'ApiGetAddress',
            immediate: true
        },
        range(newVal)
        {
            this.karaokes = this.karaokes_tmp.filter((value,index,array) => {
                return array[index].RENT_COST_MAX >= newVal[1]
            })
        },
        selected_province(newVal)
        {
            this.district = null
        }
    },
    ...
    methods:{
        ...
        async ApiGetAddress()
        {
            ...
            if(this.$route.query.ID_DISTRICT != null)
            {
        ...
        search()
        {
            this.$router.push('/search?ID_PROVINCE='+this.selected_province+'&ID_DISTRICT='+this.district)
        }
    },
    created()
    {
