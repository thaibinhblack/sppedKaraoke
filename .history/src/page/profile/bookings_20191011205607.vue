        ...
        <v-col cols="12" sm="9" md="10">
            <v-card>
                ...
                >
                    ...
                    <template v-slot:item.TOTAL="{item}">
                        <span v-if="item.STATUS == 1">Chờ xác nhận</span>
                        <span v-else-if="item.STATUS == 2"> Đã nhận phòng </span>
                        <span v-else-if="item.STATUS == 3"> Đã nhận phòng </span>
                        <span v-else-if="item.STATUS == 4"> Đã hủy </span>
...
export default {
    data()
    {
        return {
            ...
            headers: [
                ...
                {text: 'Trạng thái', value: 'SATUS', align: 'center'}
    ...
    methods: {
        ...
        getBooking()
        {
            this.$http.get('http://127.0.0.1:8000/api/v1/' + 'booking?api_token='+this.$session.get('token')+'&status='+this.status.value)
            .then((response) => {
