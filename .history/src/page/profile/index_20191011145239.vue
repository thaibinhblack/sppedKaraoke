                    ...
                    <v-col cols="12" sm="12" style="padding-bottom: 0px;"> 
                        ...
                        <ul class="list-option">
                            ...
                            to="/profile/edit-account"
                             :class="[isActive && 'active']">Cài đặt tài khoản</router-link></li>
                        </ul>
                    </v-col>
                </v-row>
            </v-container>
...
export default {
    ...
    methods: {
        ...
        ManagerKaraoke()
        {
            this.$http.get(this.$store.state.API_URL + 'user/'+this.user.UUID_USER+'/manager-karaoke?api_token='+this.$session.get('token'))
            .then((response) => {
                this.ApiCheckToken()
            })
        }
...
.list-option li a.active {padding-bottom: 10px;border-bottom: 3px #f68a39 solid}
.list-option li a {color: #a3acb9}
.list-option li a.active,.list-option li a:hover {color: #333;;}
