            ...
            <v-container grid-list-xs>
                    <v-row>
                        ...
                        <v-col cols="12" sm="5" md="6" style="text-align:right">
                            <button @click="ManagerKaraoke()" v-if="user.UUID_RULE != 'manager-2019'" class="btn btn-manager">Trở thành chủ nhà</button>
                        </v-col>
...
export default {
    ...
    methods: {
        ...
        ManagerKaraoke()
        {
            this.$http.post(this.$store.state.API_URL + 'user/'+this.user.UUID_USER+'/manager-karaoke?api_token='+this.$session.get('token'))
            .then((response) => {
                this.ApiCheckToken()
            })
        }
