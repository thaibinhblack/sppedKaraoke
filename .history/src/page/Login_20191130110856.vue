            ...
            <v-row class="bg-header-top">
                <v-col cols="12" md=6 class="form-login">
                    <div class="form-group-login">
                        <v-alert :type="message.type" v-if="message.text != null" style="width: 100%" :value="true">
                                {{message.text}}
                        </v-alert>
...
export default {
    ...
    methods: {
        ...
        async Login()
        {
            ...
            await this.$http.post(this.$store.state.API_URL + 'user/login',data)
            .then((response) => {
                if(response.data.success != false)
                {   
                    this.$cookies.config(new Date().toUTCString())
                    this.$cookies.set("token",response.data.data,60 * 60 * 24 * 30)
                    this.$store.state.token = response.data.data
                    this.$session.set('token', response.data.data)
                    this.message.type = 'success'
                    this.message.text = response.data.message
                    this.$router.push('/profile/edit-account/info')
                }
                else
                {
