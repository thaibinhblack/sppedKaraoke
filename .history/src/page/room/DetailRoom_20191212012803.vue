    ...
    <v-overlay :value="$store.state.loading" style="color:#fff !important;">
        ...
        <v-dialog v-model="dialog_thanhtoan" persistent max-width="600px">
            <v-card v-if="booking_thanhtoan.STATUS == 2">
                <form style="padding:15px;" @submit.prevent="api_thanh_toan_trcutiep()">
                    <h3>Thông tin thanh toán</h3>
                    <v-text-field
                        name="DATE_BOOK"
                        label="Ngày đặt phòng đặt phòng"
                        v-model="booking_thanhtoan.DATE_BOOK"
                        disabled
                    ></v-text-field>
                    <v-text-field
                        name="TIME_START"
                    ...
                    ></v-text-field>
                     <v-text-field
                        ...
                        v-model="booking_thanhtoan.TIME_END"
                        disabled
                    ></v-text-field>
                    <v-text-field
                        name="CODE"
                        ...
                        :disabled="check_code_promotion.success"
                        @click:append="check_code()"
                    ></v-text-field>
                    <p>Tổng thời gian: {{total}} phút</p>
                    <p>Thành tiền:  {{Math.floor((room.RENT_COST/60) * total).toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,') }} vnđ</p>
                    <button class="button btn-add" @click="dialog_thanhtoan = false">Đóng</button> 
                    <button type="submit" class="btn btn-add" style="margin-left:5px">Thanh toán</button>
                </form>
            </v-card>
        </v-dialog>
</div>
...
export default {
    ...
    data()
    {
        return {
            ...
            booking_thanhtoan: {
                RENT_COST: 0,
                TIME_END: new Date().getHours()+':'+new Date().getMinutes()+':00'
            },
            CODE_PROMOTION: "",
            check_code_promotion: {
                success: false
            }
        }
    },
    computed: {
        computedDateFormatted () {
         ...
         total(){
            if(this.booking_thanhtoan.STATUS == 2)
            {
    ...
    watch: {
        dialog_thanhtoan(val)
        {
            if(val == true)
            {
                 this.booking_thanhtoan.TIME_END = new Date().getHours()+':'+new Date().getMinutes()+':00'
            }
        },
      date (val) {
    ...
    methods: {
        ...
        {
            this.axios.get(this.$store.state.API_URL + 'booking/'+this.booking_thanhtoan.UUID_BOOKING+'/paypal').then((response) => {
                console.log(response.data)
            })
        },
        check_code()
        {
            this.axios.get(this.$store.state.API_URL + 'promotion/'+this.room.UUID_BAR_KARAOKE+'/check?code_promotion='+this.CODE_PROMOTION).then((response) => {
                console.log(response.data)
                this.check_code_promotion = response.data
