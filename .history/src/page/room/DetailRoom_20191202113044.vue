    ...
    watch: {
      ...
      rating_room(newVal)
      {
          if(newVal > 0 )
          {
              ...
              {
                  this.axios.get(this.$store.state.API_URL + 'room/'+this.$route.params.UUID_ROOM_BAR_KARAOKE+'/rating?api_token='+this.$cookies.get('token')+'&UUID_ROOM_BAR_KARAOKE='+this.$route.params.UUID_ROOM_BAR_KARAOKE)
                  .then((response) => {
