    ...
    methods:{
        ...
        renderBanner()
        {
            ...
            const app = this;
              storageRef.child(file.name).put(file).then(function(snapshot) {
                  ...
                  storage.child(file.name).getDownloadURL().then((url) => { 
                      app.karaoke.BANNER_BAR_KARAOKE = url
                  })
                })
            // this.
        ...
        renderLogo()
        {
            ...
            storageRef.child(file.name).put(file).then(function(snapshot) {
                var storage = firebase.storage().ref('');
                storage.child(file.name).getDownloadURL().then((url) => {
                    app.karaoke.LOGO_BAR_KARAOKE = url
                })
            })
        ...
        async updateKaraoke()
        {
            ...
            await this.$http.post(this.$store.state.API_URL + 'karaoke/'+this.$route.params.uuid+'/update?api_token='+this.$session.get('token'),data)
            .then((response) => {  
