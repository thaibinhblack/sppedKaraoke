                ...
                <v-card>
                    <v-card-title primary-title>
                        <v-row>
                            ...
                            <v-col cols="12" sm="12">
                                <button class="btn-add"> <router-link to='/manager-karaoke/karaoke/add-karaoke'><v-icon>mdi-plus</v-icon> Thêm chi nhánh mới</router-link></button>
                            </v-col>
                        </v-row>
                    ...
                    <v-card-text>
                        ...
                        >
                            ...
                            <template v-slot:item.ACTION ="{item}">
                               <router-link :to="'/manager-karaoke/karaoke/'+item.UUID_BAR_KARAOKE"><v-icon>mdi-chevron-right</v-icon> next</router-link>
                            </template>
                        </v-data-table>
                    </v-card-text>
                </v-card>
...
export default {
    ...
    methods: {
        ApiGetKaraoke()
        {
            ...
            {
                axios.get(this.$store.state.API_URL + 'karaoke?user_create='+this.$session.get('user').EMAIL).then((response) => {
                    this.karaokes = response.data
                })
            }
            else
            {
                axios.get(this.$store.state.API_URL + 'karaoke?user_create='+this.$session.get('user_social').email).then((response) => {
                    this.karaokes = response.data
                })
            }
