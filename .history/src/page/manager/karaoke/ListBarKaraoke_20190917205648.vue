...
export default {
    ...
    methods: {
        ApiGetKaraoke()
        {
            ...
            {
                this.$http.get(this.$store.state.API_URL + 'karaoke?user_create='+this.$session.get('user_social').email).then((response) => {
                    this.karaokes = response.data
                })
            }
    ...
    {
        this.ApiGetKaraoke()
        console.log(this.$store.state.ModuleManagerKaraoke.user)
    }
}
</script>