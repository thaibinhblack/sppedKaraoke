<template>
    <div id="page-add-room" style="padding:15px;">
        <v-row>
            ...
            <attribute-room v-if="room.create == true" :room="room" />
...
export default {
    ...
    methods: {
        ...
        createRoom()
        {
            if(this.room.create == false)
            {
                this.room.UUID_ROOM_BAR_KARAOKE = uuid.v4()
                if(this.$session.has("user"))
                {
                    this.room.USER_CREATE = this.$session.get("user").EMAIL
                }
                else
                {
                    this.room.USER_CREATE = this.$session.get("user_social").email
                }
            ...
            {
                this.$http.put(this.$store.state.API_URL + 'room/'+this.room.UUID_ROOM_BAR_KARAOKE).then(() => {
                    this.message.type = 'success'
