...
<div id="page-add-promotion" style="padding: 15px;">
    <v-row>
        ...
        <v-col cols="12" sm="12">
            <v-form @submit.prevent="update_promotion()">
                <v-card>
                    <v-card-text>
                        
                        <v-row>
                            <v-col cols="12" sm="12" class="banner">
                                    <v-img v-if="promotion.BANNER_PROMOTION != null" :src="$store.state.PUBLIC_URL + promotion.BANNER_PROMOTION" width="100%" height="auto" style="max-height:300px"></v-img>
...
export default {
    ...
    methods: {
        ...
        update_promotion()
        {
            ...
            this.$http.post(this.$store.state.API_URL +'promotion/'+this.$route.params.UUID_PROMOTION+'?api_token='+this.$cookies.get('token'),data)
            .then((response) => {
