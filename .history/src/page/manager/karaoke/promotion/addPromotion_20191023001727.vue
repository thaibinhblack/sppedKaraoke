                    ...
                    <v-form @submit.prevent="createPromotion()">
                       <v-row>
                           <v-col cols="12" sm="12" class="banner">
                                <v-img v-if="banner != null" :src="banner" height="180px"></v-img>
                            ...
                            <v-col class="12" sm="4">
                                <v-menu
                                    ...
                                    <template v-slot:activator="{ on }">
                                        ...
                                        @blur="promotion.DATE_STARTED = parseDate(promotion.DATE_STARTED)"
                                        required
                                        v-on="on"
                                        ></v-text-field>
                                    </template>
                            ...
                            <v-col class="12" sm="4">
                                <v-menu
                                    ...
                                    <template v-slot:activator="{ on }">
                                        ...
                                        persistent-hint
                                        prepend-icon="mdi-calendar-range"
                                        @blur="promotion.DATE_END = parseDate(promotion.DATE_END)"
                                        required
                                        v-on="on"
                                        ></v-text-field>
                                    </template>
...
export default {
    ...
    watch: {
      date (val) {
        this.promotion.DATE_END = this.formatDate(this.date)
      },
      date_start(val)
      {
    ...
    methods: {
        ...
        createPromotion()
        {
            ...
            data.append("VALUE_PROMOTION",this.promotion.VALUE_PROMOTION)
            data.append("DATE_STARTED",this.promotion.DATE_STARTED)
            data.append("DATE_END",this.promotion.DATE_END)
            this.$http.post(this.$store.state.API_URL +'promotion?api_token='+this.$session.get('token'))
            .then((response) => {
