                    ...
                    <v-row style="margin:0 -5px">
                        <v-col cols="12" sm="3" md="2" v-for="(image,i) in images" :key="i" style="padding:5px;">
                             <div class="item-image">
                                     <v-img width="100%" height="150px" :src="url"></v-img>
                                </div>
                        </v-col>
                    </v-row>
                </section>
        ...
        ApiDetailKaraoke()
        {
            ...
            const app = this;
             this.$http.get(this.$store.state.API_URL + 'karaoke?safe_url='+this.$route.params.safeurl).then((response) => {
                ...
                this.$http.get(this.$store.state.API_URL + '    /'+response.data.UUID_BAR_KARAOKE+'/view').then(() => {
        ...
        ApiMedia(UUID)
        {
            this.$http.get(this.$store.state.API_URL + 'image/'+UUID+'?type=UUID_BAR_KARAOKE').then((response) => {
                ...
                response.data.forEach((image) => {
                    const url_image = image.TYPE == 'firebase'? image.URL_IMAGE: this.$store.state.PUBLIC_URL + this.image.URL_IMAGE;
                    photos.push({
