        ...
        <v-row>
            ...
            <v-col cols="12" sm="8" md="9">
                ...
                <section style="margin-top:25px;" v-if="rooms.length > 0">
                    ...
                    <v-row style="margin:0 -5px">
                        <v-col cols="12" sm="4" md="3"  v-for="(r,i) in rooms" :key="i" style="padding:5px;">
                           <div class="item-room">
                                <router-link :to="'/karaoke/'+$route.params.safeurl+'/'+r.UUID_ROOM_BAR_KARAOKE"><v-img :src="$store.state.PUBLIC_URL + r.IMAGE_ROOM_BAR_KARAOKE" height="180px"></v-img></router-link>
                                <div class="content-room">
                                    ...
                                    <p><v-icon size="12" color="yellow" v-for="i in r.STAR_RATING" :key="i">mdi-star</v-icon> <small>({{r.NUMBER_RATED}} đánh giá), {{r.VIEW_ROOM}} lượt xem</small></p>
                                </div>
                           </div>
                        </v-col>
                    </v-row>
    ...
    methods:{
        async ApiDetailKaraoke()
        {
            ...
            await this.$http.get(this.$store.state.API_URL + 'karaoke?safe_url='+this.$route.params.safeurl).then((response) => {
                ...
                app.$http.get(app.$store.state.API_URL + 'karaoke/'+response.data.UUID_BAR_KARAOKE+'/view').then(() => {
                    app.karaoke.VIEW = app.karaoke.VIEW + 1
                })
