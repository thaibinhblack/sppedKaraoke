            ...
            <v-col cols="12" sm="5" md="4">
                <div class="item-karaoke" v-for="(karaoke,index) in karaoke_star" :key="index">
                    <div class="image-karaoke">
                        <v-img :src="$store.state.PUBLIC_URL + karaoke.BANNER_BAR_KARAOKE"></v-img>
                    </div>
                </div>
            </v-col>
...
export default {
    ...
    methods:{
        ...
        {
            this.$http.get(this.$store.state.API_URL + 'karaoke?sort=star&UUID_BAR_KARAOKE='+UUID).then((response) => {
