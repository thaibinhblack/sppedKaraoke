        ...
        <v-container grid-list-xs>
            <v-row>
                <v-col cols="12" sm="12" class="group-link" style="text-align:left;">
                    <ul class="list-link">
                        <li><router-link :to="'/karaoke?ID_PROVINCE='+address.ID_PROVINCE">{{address.NAME_PROVINCE}}</router-link></li>
    ...
    methods:{
        ApiGetAddress()
        {
            if(this.$route.query.ID_DISTRICT)
            {
                ...
                this.$http.get(this.$store.state.API_URL + 'district/'+this.$route.query.ID_PROVINCE).then((response) => {
                   this.address = response.data
                })
