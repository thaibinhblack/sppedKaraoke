...
export default {
    ...
    methods: {
        ...
        ApiGetKaraokeMap()
        {
            this.$http.get(this.$store.state.API_URL + 'karaoke?type=objectid').then((response) => {
    ...
    async created()
    {
        ...
        if(this.$route.query.UUID_BAR_KARAOKE){
            await this.$http.get(this.$store.state.API_URL + 'karaoke/'+this.$route.query.UUID_BAR_KARAOKE).then((response) => {
               await this.commitgetFeature(response.data.OBJECTID)
            })
        }
    }
}
